...
router.post("/", authMiddleware, upload.single('imagen'), validacionUsuario, async (req, res) => {
    const { username, email, password_hashed, is_admin, role } = req.body;
...
router.put("/:id", authMiddleware, upload.single('imagen'), validacionUsuario, async (req, res) => {
    const { id } = req.params;
    const { username, email, password_hashed, is_admin, role } = req.body;
